package com.cheliji.ai.knowledgeflow.framework.distributedid;
...
/**
 * 分布式 Snowflake 初始化器
 * 从 Redis 获取 workerId 和 datacenterId，注册到 Hutool 的 IdUtil 中
 */
@Slf4j
@Component
    ...
    public void init() {
        ...
        try {
            ...
            if(CollUtil.isEmpty(result) || result.size() != 2) {
                throw new RuntimeException("从 Redis 获取 workerId 和 datacenterId 失败") ;
